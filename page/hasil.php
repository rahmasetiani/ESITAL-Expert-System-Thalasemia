...
// Fetch selected symptoms with their weights
$gejalaQuery = "SELECT g.kodegejala, g.namagejala, g.bobot 
...
while ($case = $caseResult->fetch_assoc()) {
    ...
    foreach ($selectedGejala as $gejala) {
        if (in_array($gejala['kode'], $caseGejala)) {
            $matchingBobot += $gejala['bobot'];
        }
    }

    // Calculate similarity as matching weight / total weight of the case
    $similarityScore = $matchingBobot / $caseTotalBobot;

    $casesSimilarity[] = [
        'kodebasiskasus' => $case['kodebasiskasus'],
        'namapenyakit' => $case['namapenyakit'],
        'matching_bobot' => $matchingBobot, // Pembilang
        'total_bobot' => $caseTotalBobot, // Penyebut
    ...
    <section class="diagnosis-section" style="background-color: #f8f9fa; padding: 20px; border-radius: 8px;">
                                                ...
                                                <?php if (!empty($selectedGejala)): ?>
                                                    <ol style="margin: 0; padding-left: 20px;">
                                                        <?php foreach ($selectedGejala as $gejala): ?>
                                                            <li><?php echo htmlspecialchars($gejala['kode']); ?> - <?php echo htmlspecialchars($gejala['nama']); ?> (Bobot: <?php echo $gejala['bobot']; ?>)</li>
                                                        <?php endforeach; ?>
                                                    </ol>
                                                <?php else: ?>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <p style="color: black;">
                    <strong>Similarity:</strong> <?php echo round($bestMatch['similarity'] * 100, 2); ?>%
                    (<?php echo $bestMatch['matching_bobot']; ?> bobot matching / <?php echo $bestMatch['total_bobot']; ?> total bobot)
                </p>
                ...
                <ul class="list-group">
                    <?php foreach ($casesSimilarity as $case): ?>
                        <li class="list-group-item">
                            ...
                            (<?php echo $case['matching_bobot']; ?> bobot matching / <?php echo $case['total_bobot']; ?> total bobot)
                        </li>
                    <?php endforeach; ?>
                </ul>
            </div>
